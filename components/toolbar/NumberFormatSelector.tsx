  ...
  return (
    <Dropdown
      trigger={
        <button
          type="button"
          className="flex items-center gap-1 h-7 px-2 rounded text-xs border border-gray-200 hover:bg-gray-50 min-w-[90px] justify-between"
        >
    ...
    >
      {NUMBER_FORMATS.map((fmt) => (
        <DropdownItem key={fmt.id} active={fmt.id === value} onClick={() => onChange(fmt.id)}>
